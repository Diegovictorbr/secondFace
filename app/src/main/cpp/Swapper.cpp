...
void Swapper::testes() {
    //getEqualizedFaces retorna pontos externos igualados de dois rostos baseado em um deles. Os pontos igualados s�o importantes
    //para a triangula��o de Delaunay ser feita a partir dos mesmos pontos externos
    vector<vector<Point2f>> equalizedFaces1 = getEqualizedFaces(facialLandmarks[0],
                                                                facialLandmarks[1]);
    vector<vector<Point2f>> equalizedFaces2 = getEqualizedFaces(facialLandmarks[1],
                                                                facialLandmarks[0]);

    //faceTriangleIndexes guardam vetores de 3 inteiros, onde cada posi��o guarda um �ndice
    //de equalizedFaces (referenciando o ponto guardado com o �ndice)
    vector<vector<int>> firstFaceTriangleIndexes;
    vector<vector<int>> secondFaceTriangleIndexes;

    //Chamada � fun��o que calcula os tri�ngulos de Delaunay e preenche os �ndices em facetriangleIndexes
    //cada vetor com 3 �ndices em faceTriangleIndexes diz respeito a tr�s pontos em equalizedFaces
    ...
    //warpEquivalentTriangles equivale as regi�es triangulares faciais dos rostos e chama warpFaceTriangles passando os tri�ngulos equivalentes.
...
//srcFace � o vetor de pontos do rosto a ser equalizado, em dstFace. Os dois vetores retornados
//devem ser usados nas transforma��es faciais com warpAffine.
vector<vector<Point2f>>
Swapper::getEqualizedFaces(vector<Point2f> &srcFace, vector<Point2f> &dstFace) {
    ...
    //Capturando �ndices dos pontos faciais externos do primeiro rosto
...
void Swapper::calculateDelaunayTriangles(vector<vector<Point2f>> &equalizedFaces,
        ...
        if (rect.contains(pts[0]) && rect.contains(pts[1]) && rect.contains(pts[2])) {
            for (int j = 0; j < 3; j++)
                for (int k = 0; k < equalizedFaces[0].size(); k++)
                    if (abs(pts[j].x - equalizedFaces[0][k].x) < 1.0 &&
                        abs(pts[j].y - equalizedFaces[0][k].y) < 1.0)
                        triangleIndex[j] = k;
...
void Swapper::warpEquivalentTriangles(vector<vector<Point2f>> &equalizedFaces,
                                      vector<vector<int>> &faceTriangleIndexes) {
...
void
Swapper::warpFaceTriangles(vector<Point2f> &srcFaceTriangles, vector<Point2f> &dstFaceTriangles) {
    ...
    for (int i = 0; i < 3; i++) {
        trueTriangle1.push_back(Point2f(srcFaceTriangles[i].x - triangle1BRect.x,
                                        srcFaceTriangles[i].y - triangle1BRect.y));
        trueTriangle2.push_back(Point2f(dstFaceTriangles[i].x - triangle2BRect.x,
                                        dstFaceTriangles[i].y - triangle2BRect.y));
        trueTriangle2Int.push_back(Point(dstFaceTriangles[i].x - triangle2BRect.x,
                                         dstFaceTriangles[i].y - triangle2BRect.y));
    }
    ...
    multiply(originalFrame(triangle2BRect), Scalar(1.0, 1.0, 1.0) - mask,
             originalFrame(triangle2BRect));
...
void Swapper::smoothSwappedFaces(vector<Point2f> &dstFace) {
    ...
    seamlessClone(originalFrame(boundingRect(hull8U)), originalFrameBackup(boundingRect(hull8U)),
                  mask(boundingRect(hull8U)), center, originalFrame(boundingRect(hull8U)),
                  NORMAL_CLONE);
}